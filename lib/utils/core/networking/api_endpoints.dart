/// this class is used for api endpoints for api calls
class ApiEndpoints {
  static const baseUrl = "http://13.200.156.40/api/v1";
  static const googleMaps = "http://maps.google.com?q=";
  static const googleMapsApiKey = "AIzaSyAKDcx6BLLWbov0NUuHlVtp8DMsnnlcI-s";
/*  static const googleMapsApiKey3 = "AIzaSyD77iSXiorGVMdVXMwrkIBsFdEjePBdtTU";
  static const googleMapsApiKey = "AIzaSyAtfbdK1Uk1Zr1keTVOQO2Y-V3ZwaV3ORQ";
  static const googleMapsApiKey4 = "AIzaSyD77iSXiorGVMdVXMwrkIBsFdEjePBdtTU";*/
  static const googleMapsApi = "https://www.google.com/maps/search/?api=1&query=";
  static const placeApi = "https://fyndeg.tech/api/map?input=";
  static const googlePlaceSearchFromLatLagApi = "https://maps.googleapis.com/maps/api/geocode/json";

  ///log_in
  static const login = "$baseUrl/User/Login";

String buildLocationRedirectionString({required num lat, required  num lng}){
  String location = "$googleMaps$lat,$lng";
  return location;
}



  ///dashboard





}
